...
abstract class CommandBase protected constructor(val command: String) {
    ...
    companion object {
        ...
        @JvmStatic
        @Deprecated("Deprecated by paper brigadier; change plugin-name instead", level = DeprecationLevel.ERROR)
    ...
    val eligibleArguments: List<Pair<CommandArgument<*>, CommandArgument<*>>>
        get() {
            fun CommandArgument<*>.optionalTree(): List<CommandArgument<*>> {
                ...
                val arguments = arguments.filter { it.isOptional }
            ...
            fun CommandArgument<*>.skipTree(): List<CommandArgument<*>> {
                ...
                list.addAll(arguments)
                arguments.filter { it.isOptional }.forEach { 
            ...
            val optional = arguments.filter { it.isOptional }.flatMap { base -> base.optionalTree().map { base to it } }
            set.addAll(optional)
            
            // Adds all skippable 
            val skip = arguments.filter { it.isOptional }.flatMap { base -> base.skipTree().map { base to it } }
            set.addAll(skip)
